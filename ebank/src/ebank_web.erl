...
convert_tuples_to_records(Accounts) ->
   Listofaccounts = lists:map(fun(X) -> #account{id = element(2,X), details = 
...
get_balance(Name, Pin, Accounts) ->
   Account = lists:filter(fun(X) -> X#account.details#accountDetails.name =:= Name andalso X#account.details#accountDetails.pin =:= Pin end, Accounts).
